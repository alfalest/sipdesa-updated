        ...
        <div class="card round-1">
          ...
          <div class="card-body px-1 px-sm-2 px-md-3 px-lg-4">
            <div class="row">
              <div class="col-12">
                <div class="pb-3 px-1">
                  <a href="<?= base_url() ?>/penduduk/surat-kematian-edit" class="px-3 btn btn-primary btn-sm rounded-pill"><i class="mr-2 fas fa-plus"></i>Surat Baru</a>
            ...
            <div class="row">
              <div class="col-4">
                <a class="border shadow-sm btn btn-block round-1 mb-2 <?= (isset($status_state) && $status_state == 1) ? 'btn-primary' : 'btn-light text-primary ' ?>" href="<?= base_url() ?>/penduduk/surat-kematian/diproses">
                  Diproses <i class="fas fa-angle-right"></i>
                </a>
              </div>
              <div class="col-4">
                <a class="border shadow-sm btn btn-block round-1 mb-2 <?= (isset($status_state) && $status_state == 2) ? 'btn-primary' : 'btn-light text-primary ' ?>" href="<?= base_url() ?>/penduduk/surat-kematian/diterima">
                  Diterima <i class="fas fa-angle-right"></i>
                </a>
              </div>
              <div class="col-4">
                <a class="border shadow-sm btn btn-block round-1 mb-2 <?= (isset($status_state) && $status_state == 3) ? 'btn-primary' : 'btn-light text-primary ' ?>" href="<?= base_url() ?>/penduduk/surat-kematian/ditolak">
                    ...
                    <div class="position-relative border shadow-sm round-1 d-flex flex-column mb-3 p-2">
                      <?php if ($value['belum_dilihat'] == 1) : ?>
                        <div class="position-absolute" style="top:-8px; left:-8px;">
                          <a href="<?= base_url() ?>/penduduk/surat-kematian/<?= $value['id_surat_kematian'] ?>">
                      ...
                      <div class="d-flex justify-content-between border-bottom pb-2">
                        ...
                        <div class="">
                          <?php if ($value['status_surat_kematian'] == 1) : ?>
                            <a href="<?= base_url() ?>/penduduk/surat-kematian-edit/<?= $value['id_surat_kematian'] ?>" class="px-3 py-0 btn btn-sm btn-success round-1 text-sm"><i class="fas fa-edit"></i> Edit</a>
                          <?php endif; ?>
                        </div>
                      </div>
                      <a href="<?= base_url() ?>/penduduk/surat-kematian-lihat/<?= $value['id_surat_kematian'] ?>">
                        <div class="row text-dark">
                          <div class="col-6 col-lg-3 mb-1">
                            <span class="text-muted font-weight-bold">Nama Orang Yang Wafat</span><br>
                            <p>Nama : <?= $value['nama_wafat'] ?></p>
                            <p>Tanggal Wafat : <?= $value['tanggal_wafat'] ?></p>
                          </div>
                        </div>
                      </a>
                    </div>
