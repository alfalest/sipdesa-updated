...
class Surat_pemakaman_lihat extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if ($param != '' && $id_user) {
            $data_surat_pemakaman = $this->tbl_surat_pemakaman->where(['id_surat_pemakaman' => intval($param), 'id_user' => $id_user])->first();
        }

        if ($data_surat_pemakaman) {

            if (isset($data_surat_pemakaman['belum_dilihat']) && $data_surat_pemakaman['belum_dilihat'] == 1) {
                if ($this->tbl_surat_pemakaman->set(['belum_dilihat' => null])->where(['id_surat_pemakaman' => intval($param), 'id_user' => $id_user])->update()) {
                    $data_surat_pemakaman['belum_dilihat'] = null;
                }
            }
        ...
        if ($this->session->getFlashdata('inp_val')) {
            $this->data['data_surat_pemakaman'] = $this->session->getFlashdata('inp_val');
            if (isset($data_surat_pemakaman['id_perubahan_kk']) && $data_surat_pemakaman['id_perubahan_kk'] != '') {
                $this->data['data_surat_pemakaman']['id_perubahan_kk'] = $data_surat_pemakaman['id_perubahan_kk'];
            }
        }
