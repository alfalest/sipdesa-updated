...
use Config\Paths;
class Surat_pengantar_nikah_lihat extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if($param != '' && $id_user){
            $data_surat_pengantar_nikah = $this->tbl_surat_pengantar_nikah->where(['id_surat_pengantar_nikah' => intval($param), 'id_user' => $id_user])->first();
        }

        if($data_surat_pengantar_nikah){
            if(isset($data_surat_pengantar_nikah['belum_dilihat']) && $data_surat_pengantar_nikah['belum_dilihat'] == 1){
               if($this->tbl_surat_pengantar_nikah->set(['belum_dilihat' => null])->where(['id_surat_pengantar_nikah'=> intval($param) , 'id_user' => $id_user])->update()){
                    $data_surat_pengantar_nikah['belum_dilihat'] = null;
               }
            }
        ...
        if($this->session->getFlashdata('inp_val')){
            $this->data['data_surat_pengantar_nikah'] = $this->session->getFlashdata('inp_val');
            if(isset($data_surat_pengantar_nikah['id_perubahan_kk']) && $data_surat_pengantar_nikah['id_perubahan_kk'] != ''){
                $this->data['data_surat_pengantar_nikah']['id_perubahan_kk'] = $data_surat_pengantar_nikah['id_perubahan_kk'];
            }
        }

        if(!isset($this->data['data_surat_pengantar_nikah']['link_foto_kk']) || $this->data['data_surat_pengantar_nikah']['link_foto_kk'] == ''){
            if(isset($data_surat_pengantar_nikah['link_foto_kk'])){
                $this->data['data_surat_pengantar_nikah']['link_foto_kk'] = $data_surat_pengantar_nikah['link_foto_kk'];
            }
        }
        ...
        if(isset($this->data['data_surat_pengantar_nikah'])){
            foreach ($this->arr_postfix as $key => $value) {
                $this->data['data_surat_pengantar_nikah'] = $this->wilayah($this->data['data_surat_pengantar_nikah'], $key);
                $this->data['data_surat_pengantar_nikah'] = $this->kode_kata($this->data['data_surat_pengantar_nikah'], $key);
                $this->data['data_surat_pengantar_nikah'] = $this->negara($this->data['data_surat_pengantar_nikah'], $key);
                $this->data['data_surat_pengantar_nikah'] = $this->kode_kata($this->data['data_surat_pengantar_nikah']);
            }
        }
        ...
        $this->data['v']['page'] = 'surat_pengantar_nikah_lihat';
        $this->data['v']['script'] = 'surat_pengantar_nikah_lihat';
