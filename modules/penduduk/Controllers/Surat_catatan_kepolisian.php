...
class Surat_catatan_kepolisian extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        $this->data['kk_proses'] = $this->Tbl_surat_catatan_kepolisian->where(['status_surat_catatan_kepolisian' => $status_perubahan, 'id_user' => $id_user])->orderBy("update_time", "DESC")->paginate(5, 'sk');

        $data_informasi_layanan = $this->tbl_informasi_layanan->where(['jenis_informasi' => 'surat_pengantar_keterangan_catatan_kepolisian', 'default_informasi' => 1])->first();
        if ($data_informasi_layanan) {
            $this->data['data_informasi_layanan'] = $data_informasi_layanan;
        } else {
            $data_informasi_layanan = $this->tbl_informasi_layanan->where(['jenis_informasi' => 'surat_pengantar_keterangan_catatan_kepolisian'])->first();
            if ($data_informasi_layanan) {
                $this->data['data_informasi_layanan'] = $data_informasi_layanan;
            }
        }
