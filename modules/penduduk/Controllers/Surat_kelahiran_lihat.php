...
use Config\Paths;
class Surat_kelahiran_lihat extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if($param != '' && $id_user){
            $data_surat_kelahiran = $this->tbl_surat_kelahiran->where(['id_surat_kelahiran' => intval($param), 'id_user' => $id_user])->first();
        }

        if($data_surat_kelahiran){
            if(isset($data_surat_kelahiran['belum_dilihat']) && $data_surat_kelahiran['belum_dilihat'] == 1){
               if($this->tbl_surat_kelahiran->set(['belum_dilihat' => null])->where(['id_surat_kelahiran'=> intval($param) , 'id_user' => $id_user])->update()){
                    $data_surat_kelahiran['belum_dilihat'] = null;
               }
            }
            $this->data['data_surat_kelahiran'] = $data_surat_kelahiran;
        }


        //cek jika ada session inp_val
        if($this->session->getFlashdata('inp_val')){
            $this->data['data_surat_kelahiran'] = $this->session->getFlashdata('inp_val');
            if(isset($data_surat_kelahiran['id_perubahan_kk']) && $data_surat_kelahiran['id_perubahan_kk'] != ''){
                $this->data['data_surat_kelahiran']['id_perubahan_kk'] = $data_surat_kelahiran['id_perubahan_kk'];
            }
        }

        if(!isset($this->data['data_surat_kelahiran']['link_foto_kk']) || $this->data['data_surat_kelahiran']['link_foto_kk'] == ''){
            if(isset($data_surat_kelahiran['link_foto_kk'])){
                $this->data['data_surat_kelahiran']['link_foto_kk'] = $data_surat_kelahiran['link_foto_kk'];
            }
        }

        //ambil data wilayah untuk membuat option
        if(isset($this->data['data_surat_kelahiran'])){
            foreach ($this->arr_postfix as $key => $value) {
                $this->data['data_surat_kelahiran'] = $this->wilayah($this->data['data_surat_kelahiran'], $key);
                $this->data['data_surat_kelahiran'] = $this->kode_kata($this->data['data_surat_kelahiran'], $key);
                $this->data['data_surat_kelahiran'] = $this->negara($this->data['data_surat_kelahiran'], $key);
                $this->data['data_surat_kelahiran'] = $this->kode_kata($this->data['data_surat_kelahiran']);
            }
        }
        ...
        $this->data['v']['page'] = 'surat_kelahiran_lihat';
        $this->data['v']['script'] = 'surat_kelahiran_lihat';
