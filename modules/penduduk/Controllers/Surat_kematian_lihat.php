...
class Surat_kematian_lihat extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        $data_surat_kematian = [];
        if ($param != '' && $id_user) {
            $data_surat_kematian = $this->tbl_surat_kematian->where(['id_surat_kematian' => intval($param), 'id_user' => $id_user])->first();
        }

        if ($data_surat_kematian) {

            if (isset($data_surat_kematian['belum_dilihat']) && $data_surat_kematian['belum_dilihat'] == 1) {
                if ($this->tbl_surat_kematian->set(['belum_dilihat' => null])->where(['id_surat_kematian' => intval($param), 'id_user' => $id_user])->update()) {
                    $data_surat_kematian['belum_dilihat'] = null;
                }
            }
        ...
        if ($this->session->getFlashdata('inp_val')) {
            $this->data['data_surat_kematian'] = $this->session->getFlashdata('inp_val');
            if (isset($data_surat_kematian['id_perubahan_kk']) && $data_surat_kematian['id_perubahan_kk'] != '') {
                $this->data['data_surat_kematian']['id_perubahan_kk'] = $data_surat_kematian['id_perubahan_kk'];
            }
        }
