...
class Surat_pencari_kerja extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        $this->data['kk_proses'] = $this->Tbl_surat_pencari_kerja->where(['status_surat_pencari_kerja' => $status_perubahan, 'id_user' => $id_user])->orderBy("update_time", "DESC")->paginate(5, 'sk');

        $data_informasi_layanan = $this->tbl_informasi_layanan->where(['jenis_informasi' => 'surat_pengantar_pencari_kerja', 'default_informasi' => 1])->first();
        if ($data_informasi_layanan) {
            $this->data['data_informasi_layanan'] = $data_informasi_layanan;
        } else {
            $data_informasi_layanan = $this->tbl_informasi_layanan->where(['jenis_informasi' => 'surat_pengantar_pencari_kerja'])->first();
            if ($data_informasi_layanan) {
                $this->data['data_informasi_layanan'] = $data_informasi_layanan;
            }
        }
