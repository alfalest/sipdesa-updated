...
class Surat_pencari_kerja_lihat extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->arr_postfix = ['_anak' => 'Anak', '_ayah' => 'Ayah', '_ibu' => 'Ibu', '_pencari_kerja' => 'Pencari Kerja'];
    }
    ...
    public function index($param = '')
    {
        ...
        if ($param != '' && $id_user) {
            $data_surat_pencari_kerja = $this->tbl_surat_pencari_kerja->where(['id_surat_pencari_kerja' => intval($param)])->first();
        }

        if ($data_surat_pencari_kerja) {
            $this->data['data_surat_pencari_kerja'] = $data_surat_pencari_kerja;

            if (isset($this->data['data_surat_pencari_kerja']['id_user'])) {
                $this->data['data_user_pemohon'] = $this->tbl_user->where(['id_user' => $this->data['data_surat_pencari_kerja']['id_user']])->first();
                $this->data['data_diri_pemohon'] = $this->tbl_data_diri->where(['id_user' => $this->data['data_surat_pencari_kerja']['id_user']])->first();
        ...
        if (!isset($this->data['data_surat_pencari_kerja']['link_foto_kk']) || $this->data['data_surat_pencari_kerja']['link_foto_kk'] == '') {
            if (isset($data_surat_pencari_kerja['link_foto_kk'])) {
                $this->data['data_surat_pencari_kerja']['link_foto_kk'] = $data_surat_pencari_kerja['link_foto_kk'];
            }
        }

        //ambil data wilayah untuk membuat option
        if (isset($this->data['data_surat_pencari_kerja'])) {
            foreach ($this->arr_postfix as $key => $value) {
                $this->data['data_surat_pencari_kerja'] = $this->wilayah($this->data['data_surat_pencari_kerja'], $key);
                $this->data['data_surat_pencari_kerja'] = $this->kode_kata($this->data['data_surat_pencari_kerja'], $key);
                $this->data['data_surat_pencari_kerja'] = $this->negara($this->data['data_surat_pencari_kerja'], $key);
                $this->data['data_surat_pencari_kerja'] = $this->kode_kata($this->data['data_surat_pencari_kerja']);
            }
        }
        ...
        $this->data['v']['page'] = 'surat_pencari_kerja_lihat';
        $this->data['v']['script'] = 'surat_pencari_kerja_lihat';
