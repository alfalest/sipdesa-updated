...
class Data_surat_print extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if ($param != '' && $id_user) {
            $data_surat = $this->tbl_data_surat->where(['id_data_surat' => intval($param)])->first();
        }

        if ($data_surat) {
            $this->data['data_surat'] = $data_surat;
            $this->data['print_preview'] = $data_surat['teks_isi_surat'];
        } else {
            return redirect()->to(base_url() . '/operator/data-surat');
        }
        ...
        $this->data['v']['page'] = 'data_surat_lihat';
        $this->data['v']['script'] = 'data_surat_lihat';
        //return view('Operator\Views\print_preview', $this->data); 
        ...
        $filename = date('y-m-d-H-i-s') . '-sipdesa.pdf';
