...
class Surat_ket_janda_editor extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        // foreach (nama_teks('surat_keterangan_ket_janda') as $key => $value) {
        //    $this->bidang_kolom[$value] = '(.....X.....)';
        // }

        //ambil kode kata untuk membuat option
        $this->list_kode_kata = ['jenis_ket_janda', 'jenis_kelamin', 'agama', 'jenis_pekerjaan', 'gol_darah'];
        ...
        if ($param != '' && $id_user) {
            $data_surat_ket_janda = $this->tbl_surat_ket_janda->where(['id_surat_ket_janda' => intval($param), 'status_surat_ket_janda' => 2])->first();
            if ($data_surat_ket_janda) {
                $data_diri = $this->tbl_data_diri->where(['id_user' => $data_surat_ket_janda['id_user']])->first();
            }
        }


        if ($data_surat_ket_janda && $data_diri) {
            $data_perubahan = $data_surat_ket_janda;
            $this->data['data_surat']['id_surat'] = $data_surat_ket_janda['id_surat_ket_janda'];
        } else {
            return redirect()->to(base_url() . '/operator/surat-ket-janda');
            exit;
        }
        ...
        $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_keterangan_status_janda', 'default_template' => 1])->first();
        if ($template_dokumen) {
        } else {
            $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_keterangan_status_janda'])->first();
        }

        if (isset($data_perubahan)) {
            //Data ket_janda
            $this->bidang_kolom['nama_ket_janda'] = $data_perubahan['nama_ket_janda'];
            $this->bidang_kolom['jenis_kelamin_ket_janda'] = $data_perubahan['jenis_kelamin_ket_janda'];
            $this->bidang_kolom['tempat_lahir_ket_janda'] =  $data_perubahan['tempat_lahir_ket_janda'];
            $this->bidang_kolom['tanggal_lahir_ket_janda'] =  $data_perubahan['tanggal_lahir_ket_janda'];
            $this->bidang_kolom['kewarganegaraan_ket_janda'] =  $data_perubahan['kewarganegaraan_ket_janda'];
            $this->bidang_kolom['agama_ket_janda'] =  $data_perubahan['agama_ket_janda'];
            $this->bidang_kolom['pekerjaan_ket_janda'] =  $data_perubahan['pekerjaan_ket_janda'];
            $this->bidang_kolom['alamat_ket_janda'] = $data_perubahan['alamat_ket_janda'];
            $this->bidang_kolom['berstatus_janda_selama'] = $data_perubahan['berstatus_janda_selama'];
        }
        if (isset($template_dokumen['template_dokumen']) && $template_dokumen['template_dokumen'] != '') {
            foreach ($this->bidang_kolom as $key => $value) {
                $data = $value;

                if ($key == 'jenis_kelamin_ket_janda') {
                    if ($value == 1) {
                        $data = "LAKI-LAKI";
                    } else {
                        $data = "PEREMPUAN";
        ...
        $this->data['v']['page'] = 'surat_ket_janda_editor';
        $this->data['v']['script'] = 'surat_ket_janda_editor';
