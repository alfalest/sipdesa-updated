...
class Surat_ket_tidak_mampu_editor_update extends BaseController
{
    public $post;
    ...
    public function index()
    {
        ...
        $set_data = [
            ...
            'jenis_surat' => 'surat_keterangan_ket_tidak_mampu',
            'teks_isi_surat' => '',
            'nomor_surat' => '',
            'nama_penanda_tangan' => '',
        ...
        if (isset($this->post['id_surat']) && $this->post['id_surat'] != '') {
            $data_surat_ket_tidak_mampu = $this->tbl_surat_ket_tidak_mampu->where(['id_surat_ket_tidak_mampu' => $this->post['id_surat'], 'status_surat_ket_tidak_mampu' => 2])->first();
            if ($data_surat_ket_tidak_mampu) {
                $id_surat_ket_tidak_mampu =  $data_surat_ket_tidak_mampu['id_surat_ket_tidak_mampu'];
                $set_data['id_surat'] = $data_surat_ket_tidak_mampu['id_surat_ket_tidak_mampu'];
            } else {
                $valid = false;
        ...
        if ($id_surat_ket_tidak_mampu != '' && $id_surat_ket_tidak_mampu > 0) {
            if (isset($id_data_surat) && $id_data_surat != '' && $this->message_info['status']) {
                ...
                return redirect()->to(base_url() . '/operator/surat-ket-tidak-mampu-editor/' . $id_surat_ket_tidak_mampu);
            }
        } else {
            //return redirect()->to( base_url().'/operator/surat-ket_tidak_mampu/diterima');
            return redirect()->to(base_url() . '/operator/surat-ket-tidak-mampu/diterima');
        }
        exit;
    }
