...
class Surat_pemakaman_editor_update extends BaseController
{
    public $post;
    ...
    public function index()
    {
        ...
        $set_data = [
            ...
            'jenis_surat' => 'surat_keterangan_pemakaman',
            'teks_isi_surat' => '',
            'nomor_surat' => '',
            'nama_penanda_tangan' => '',
        ...
        if (isset($this->post['id_surat']) && $this->post['id_surat'] != '') {
            $data_surat_pemakaman = $this->tbl_surat_pemakaman->where(['id_surat_pemakaman' => $this->post['id_surat'], 'status_surat_pemakaman' => 2])->first();
            if ($data_surat_pemakaman) {
                $id_surat_pemakaman =  $data_surat_pemakaman['id_surat_pemakaman'];
                $set_data['id_surat'] = $data_surat_pemakaman['id_surat_pemakaman'];
            } else {
                $valid = false;
        ...
        if ($id_surat_pemakaman != '' && $id_surat_pemakaman > 0) {
            if (isset($id_data_surat) && $id_data_surat != '' && $this->message_info['status']) {
                ...
                return redirect()->to(base_url() . '/operator/surat-pemakaman-editor/' . $id_surat_pemakaman);
            }
        } else {
            //return redirect()->to( base_url().'/operator/surat-pemakaman/diterima');
            return redirect()->to(base_url() . '/operator/surat-pemakaman/diterima');
        }
        exit;
    }
