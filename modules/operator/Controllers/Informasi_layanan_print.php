...
class Informasi_layanan_print extends BaseController
{
    ...
    public function index($param = '', $id_param = '')
    {
        ...
        if($param != '' && $id_param != '' && $id_user){
            $data_template = $this->tbl_informasi_layanan->where([ 'jenis_template' => str_replace('-', '_', $param) ,'id_informasi_layanan' => intval($id_param)])->first();
        }

        if($data_template){
            $this->data['data_surat'] = $data_template;
            $this->data['print_preview'] = $data_template['informasi_layanan'];
        }
        ...
        $filename = 'Template_'.$data_template['jenis_template'].'_'.$data_template['id_informasi_layanan'].'_'.str_replace('-', '', date('Y-m-d')).'.pdf';
