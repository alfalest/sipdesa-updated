...
class Surat_kematian_editor extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        // foreach (nama_teks('surat_keterangan_kematian') as $key => $value) {
        //    $this->bidang_kolom[$value] = '(.....X.....)';
        // }

        //ambil kode kata untuk membuat option
        $this->list_kode_kata = ['jenis_kematian', 'jenis_kelamin', 'agama', 'jenis_pekerjaan', 'gol_darah'];
        ...
        $data_surat_kematian = [];
        if ($param != '' && $id_user) {
            $data_surat_kematian = $this->tbl_surat_kematian->where(['id_surat_kematian' => intval($param), 'status_surat_kematian' => 2])->first();
            if ($data_surat_kematian) {
                $data_diri = $this->tbl_data_diri->where(['id_user' => $data_surat_kematian['id_user']])->first();
            }
        }


        if ($data_surat_kematian && $data_diri) {
            $data_perubahan = $data_surat_kematian;
            $this->data['data_surat']['id_surat'] = $data_surat_kematian['id_surat_kematian'];
        } else {
            return redirect()->to(base_url() . '/operator/surat-kematian');
            exit;
        }
        ...
        $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_keterangan_kematian', 'default_template' => 1])->first();
        if ($template_dokumen) {
        } else {
            $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_keterangan_kematian'])->first();
        }

        //Data Anak
        if (isset($data_perubahan)) {

            $this->bidang_kolom['hari_wafat'] = strtoupper(ubah_tanggal($data_perubahan['hari_wafat'])['hari']);
            $this->bidang_kolom['tanggal_wafat'] = date('d-m-Y', strtotime($data_perubahan['tanggal_wafat']));
            $this->bidang_kolom['jam_wafat'] =  substr($data_perubahan['jam_wafat'], 0, -3);
            $this->bidang_kolom['tempat_wafat'] = strtoupper($data_perubahan['tempat_wafat']);
            $this->bidang_kolom['penyebab_wafat'] = strtoupper($data_perubahan['penyebab_wafat']);

            //Data Kematian

            $this->bidang_kolom['nama_wafat'] = strtoupper($data_perubahan['nama_wafat']);
            $this->bidang_kolom['jenis_kelamin_wafat'] = strtoupper($data_perubahan['jenis_kelamin_wafat']);
            $this->bidang_kolom['tempat_lahir_wafat'] =  strtoupper($data_perubahan['tempat_lahir_wafat']);
            $this->bidang_kolom['tanggal_lahir_wafat'] = date('d-m-Y', strtotime($data_perubahan['tanggal_lahir_wafat']));
            $this->bidang_kolom['kewarganegaraan_wafat'] =  strtoupper($data_perubahan['kewarganegaraan_wafat']);
            $this->bidang_kolom['agama_wafat'] =  strtoupper($data_perubahan['agama_wafat']);
            $this->bidang_kolom['pekerjaan_wafat'] =  strtoupper($data_perubahan['pekerjaan_wafat']);
            $this->bidang_kolom['alamat_wafat'] =  strtoupper($data_perubahan['alamat_wafat']);
        }
        if (isset($template_dokumen['template_dokumen']) && $template_dokumen['template_dokumen'] != '') {
            foreach ($this->bidang_kolom as $key => $value) {
                $data = $value;

                if ($key == 'jenis_kelamin_wafat') {
                    if ($value == 1) {
                        $data = "LAKI-LAKI";
                    } else {
                        $data = "PEREMPUAN";
        ...
        $this->data['v']['page'] = 'surat_kematian_editor';
        $this->data['v']['script'] = 'surat_kematian_editor';
