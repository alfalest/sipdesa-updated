...
class Surat_kelahiran extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if (isset($this->get['cari']) && $this->get['cari'] != '') {
            ...
            $this->data['kk_proses'] = $this->tbl_surat_kelahiran->where(['status_surat_kelahiran' => $status_perubahan])
                ->like('nama_anak', $this->get['cari'])
                ->orLike('nama_ayah', $this->get['cari'])
                ->orLike('nama_ibu', $this->get['cari'])
                ->orLike('nik_anak', $this->get['cari'])
                ->orLike('nik_ayah', $this->get['cari'])
                ->orLike('nik_ibu', $this->get['cari'])
                ->orderBy("update_time", "ASC")
                ->paginate($per_page, 'sk');
        } else {
            $this->data['kk_proses'] = $this->tbl_surat_kelahiran->where(['status_surat_kelahiran' => $status_perubahan])->orderBy("update_time", $urutan)->paginate($per_page, 'sk');
        }

        $data_informasi_layanan = $this->tbl_informasi_layanan->where(['jenis_informasi' => 'surat_keterangan_kelahiran', 'default_informasi' => 1])->first();
        if ($data_informasi_layanan) {
            $this->data['data_informasi_layanan'] = $data_informasi_layanan;
        } else {
            $data_informasi_layanan = $this->tbl_informasi_layanan->where(['jenis_informasi' => 'surat_keterangan_kelahiran'])->first();
            if ($data_informasi_layanan) {
                $this->data['data_informasi_layanan'] = $data_informasi_layanan;
            }
        }

        // $this->data['kk_diterima'] = $this->tbl_surat_kelahiran->where(['status_perubahan_kk' => 2, 'id_user' => $id_user])->paginate(1, 'kk_diterima');
        // $this->data['kk_ditolak'] = $this->tbl_surat_kelahiran->where(['status_perubahan_kk' => 3, 'id_user' => $id_user])->paginate(1, 'kk_ditolak');
        ...
        $this->data['pager'] = $this->tbl_surat_kelahiran->pager;

        $this->data['v']['page'] = 'surat_kelahiran';
        $this->data['v']['script'] = 'surat_kelahiran';
