...
class Surat_catatan_kepolisian_update extends BaseController
{
    ...
    public function index()
    {
        ...
        $set_tiket = [
            'nomor_tiket' => '',
            'jenis_tiket' => 'surat_keterangan_catatan_kepolisian',
            'filename_surat' => 'surat-catatan-kepolisian-lihat',
            'id_surat' => '',
            'status_tiket' => 1,
            'id_user' => '',
        ...
        if (isset($this->post['id_surat_catatan_kepolisian']) && $this->post['id_surat_catatan_kepolisian'] != '') {
            $data_surat_catatan_kepolisian = $this->tbl_surat_catatan_kepolisian->where(['id_surat_catatan_kepolisian' => $this->post['id_surat_catatan_kepolisian'], 'status_surat_catatan_kepolisian' => 1])->first();
            if ($data_surat_catatan_kepolisian) {
                $id_surat_catatan_kepolisian = $this->post['id_surat_catatan_kepolisian'];
                $set_tiket['id_surat'] = $data_surat_catatan_kepolisian['id_surat_catatan_kepolisian'];
                $set_tiket['id_user'] = $data_surat_catatan_kepolisian['id_user'];
            } else {
                $valid = false;
                $this->message_info['text'] .= 'Surat catatan_kepolisian tidak ditemukan<br>';
            }
        }
        ...
        //validation
        if (isset($this->post['status_surat_catatan_kepolisian']) && in_array($this->post['status_surat_catatan_kepolisian'], [2, 3])) {
            if ($this->post['status_surat_catatan_kepolisian'] == 2) {
                if (isset($data_surat_catatan_kepolisian['id_user']) && $data_surat_catatan_kepolisian['id_user'] != 0) {
            ...
            $set_data['status_surat_catatan_kepolisian'] = $this->post['status_surat_catatan_kepolisian'];
        } else {
            $valid = false;
            $this->message_info['text'] .= 'Pilih salah satu jenis verifikasi : Terima atau Tolak<br>';
            $this->invalid_info['status_surat_catatan_kepolisian'] = 'Pilih salah satu jenis verifikasi : Terima atau Tolak';
        }
        ...
        if (isset($this->post['keterangan_operator']) &&  $this->post['keterangan_operator'] != '') {
            if (mb_strlen($this->post['keterangan_operator'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_catatan_kepolisian'] = 'Catatan melebihi 150 karakter';
            }
        }
        ...
        if (isset($this->post['keterangan_tiket']) && $this->post['keterangan_tiket'] !=  '') {
            if (mb_strlen($this->post['keterangan_tiket'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_catatan_kepolisian'] = 'Keterangan tiket melebihi 150 karakter';
            }
        }
        ...
        if ($valid) {
            if ($set_data['status_surat_catatan_kepolisian'] == 2) {
            ...
            if ($valid) {
                if ($this->tbl_surat_catatan_kepolisian->set($set_data)->where('id_surat_catatan_kepolisian', $id_surat_catatan_kepolisian)->update()) {
                    ...
                    $this->message_info['text'] .= 'Data surat catatan_kepolisian berhasil diubah<br>';
                    return redirect()->to(base_url() . '/operator/surat-catatan-kepolisian-edit/' . $id_surat_catatan_kepolisian);
                } else {
                    $this->message_info['text'] .= 'Data surat catatan_kepolisian gagal diverifikasi, periksa jika ada tiket atas surat ini<br>';
                }
            }
        }
        ...
        if ($id_surat_catatan_kepolisian != '' && $id_surat_catatan_kepolisian > 0) {
            if ($this->message_info['status']) {
                return redirect()->to(base_url() . '/operator/surat-catatan-kepolisian-lihat/' . $id_surat_catatan_kepolisian);
            } else {
                return redirect()->to(base_url() . '/operator/surat-catatan-kepolisian-edit/' . $id_surat_catatan_kepolisian);
            }
        } else {
