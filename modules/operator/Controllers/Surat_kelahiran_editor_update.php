...
use Config\Paths;
class Surat_kelahiran_editor_update extends BaseController
{
    public $post;
    ...
    public function index()
    {
        ...
        if(isset($this->post['id_surat']) && $this->post['id_surat'] != '' ){
            $data_surat_kelahiran = $this->tbl_surat_kelahiran->where(['id_surat_kelahiran' => $this->post['id_surat'] , 'status_surat_kelahiran' => 2])->first();
            if($data_surat_kelahiran){
                $id_surat_kelahiran =  $data_surat_kelahiran['id_surat_kelahiran'];
                $set_data['id_surat'] = $data_surat_kelahiran['id_surat_kelahiran'];
        ...
        if($id_surat_kelahiran != '' && $id_surat_kelahiran > 0){
            if(isset($id_data_surat) && $id_data_surat != '' && $this->message_info['status']){
               ...
               // return redirect()->to( base_url().'/operator/data-surat-lihat/'.$id_data_surat);
                ...
                return redirect()->to( base_url().'/operator/surat-kelahiran-editor/'.$id_surat_kelahiran);
            }
        } else {
            //return redirect()->to( base_url().'/operator/surat-kelahiran/diterima');
            return redirect()->to( base_url().'/operator/surat-kelahiran/diterima');
        }
        exit;
    }
