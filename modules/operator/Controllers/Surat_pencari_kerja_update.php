...
class Surat_pencari_kerja_update extends BaseController
{
    ...
    public function index()
    {
        ...
        $set_tiket = [
            'nomor_tiket' => '',
            'jenis_tiket' => 'surat_keterangan_pencari_kerja',
            'filename_surat' => 'surat-pencari-kerja-lihat',
            'id_surat' => '',
            'status_tiket' => 1,
            'id_user' => '',
        ...
        if (isset($this->post['id_surat_pencari_kerja']) && $this->post['id_surat_pencari_kerja'] != '') {
            $data_surat_pencari_kerja = $this->tbl_surat_pencari_kerja->where(['id_surat_pencari_kerja' => $this->post['id_surat_pencari_kerja'], 'status_surat_pencari_kerja' => 1])->first();
            if ($data_surat_pencari_kerja) {
                $id_surat_pencari_kerja = $this->post['id_surat_pencari_kerja'];
                $set_tiket['id_surat'] = $data_surat_pencari_kerja['id_surat_pencari_kerja'];
                $set_tiket['id_user'] = $data_surat_pencari_kerja['id_user'];
            } else {
                $valid = false;
                $this->message_info['text'] .= 'Surat pencari_kerja tidak ditemukan<br>';
            }
        }
        ...
        //validation
        if (isset($this->post['status_surat_pencari_kerja']) && in_array($this->post['status_surat_pencari_kerja'], [2, 3])) {
            if ($this->post['status_surat_pencari_kerja'] == 2) {
                if (isset($data_surat_pencari_kerja['id_user']) && $data_surat_pencari_kerja['id_user'] != 0) {
            ...
            $set_data['status_surat_pencari_kerja'] = $this->post['status_surat_pencari_kerja'];
        } else {
            $valid = false;
            $this->message_info['text'] .= 'Pilih salah satu jenis verifikasi : Terima atau Tolak<br>';
            $this->invalid_info['status_surat_pencari_kerja'] = 'Pilih salah satu jenis verifikasi : Terima atau Tolak';
        }
        ...
        if (isset($this->post['keterangan_operator']) &&  $this->post['keterangan_operator'] != '') {
            if (mb_strlen($this->post['keterangan_operator'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_pencari_kerja'] = 'Catatan melebihi 150 karakter';
            }
        }
        ...
        if (isset($this->post['keterangan_tiket']) && $this->post['keterangan_tiket'] !=  '') {
            if (mb_strlen($this->post['keterangan_tiket'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_pencari_kerja'] = 'Keterangan tiket melebihi 150 karakter';
            }
        }
        ...
        if ($valid) {
            if ($set_data['status_surat_pencari_kerja'] == 2) {
            ...
            if ($valid) {
                if ($this->tbl_surat_pencari_kerja->set($set_data)->where('id_surat_pencari_kerja', $id_surat_pencari_kerja)->update()) {
                    ...
                    $this->message_info['text'] .= 'Data surat pencari_kerja berhasil diubah<br>';
                    return redirect()->to(base_url() . '/operator/surat-pencari-kerja-edit/' . $id_surat_pencari_kerja);
                } else {
                    $this->message_info['text'] .= 'Data surat pencari_kerja gagal diverifikasi, periksa jika ada tiket atas surat ini<br>';
                }
            }
        }
        ...
        if ($id_surat_pencari_kerja != '' && $id_surat_pencari_kerja > 0) {
            if ($this->message_info['status']) {
                return redirect()->to(base_url() . '/operator/surat-pencari-kerja-lihat/' . $id_surat_pencari_kerja);
            } else {
                return redirect()->to(base_url() . '/operator/surat-pencari-kerja-edit/' . $id_surat_pencari_kerja);
            }
        } else {
