...
class Surat_pencari_kerja_editor extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        // foreach (nama_teks('surat_keterangan_pencari_kerja') as $key => $value) {
        //    $this->bidang_kolom[$value] = '(.....X.....)';
        // }

        //ambil kode kata untuk membuat option
        $this->list_kode_kata = ['jenis_pencari_kerja', 'jenis_kelamin', 'agama', 'jenis_pekerjaan', 'gol_darah'];
        ...
        if ($param != '' && $id_user) {
            $data_surat_pencari_kerja = $this->tbl_surat_pencari_kerja->where(['id_surat_pencari_kerja' => intval($param), 'status_surat_pencari_kerja' => 2])->first();
            if ($data_surat_pencari_kerja) {
                $data_diri = $this->tbl_data_diri->where(['id_user' => $data_surat_pencari_kerja['id_user']])->first();
            }
        }


        if ($data_surat_pencari_kerja && $data_diri) {
            $data_perubahan = $data_surat_pencari_kerja;
            $this->data['data_surat']['id_surat'] = $data_surat_pencari_kerja['id_surat_pencari_kerja'];
        } else {
            return redirect()->to(base_url() . '/operator/surat-pencari-kerja');
            exit;
        }
        ...
        $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_pengantar_pencari_kerja', 'default_template' => 1])->first();
        if ($template_dokumen) {
        } else {
            $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_pengantar_pencari_kerja'])->first();
        }

        if (isset($data_perubahan)) {
            //Data pencari_kerja
            $this->bidang_kolom['nama_pencari_kerja'] = $data_perubahan['nama_pencari_kerja'];
            $this->bidang_kolom['jenis_kelamin_pencari_kerja'] = $data_perubahan['jenis_kelamin_pencari_kerja'];
            $this->bidang_kolom['tanggal_lahir_pencari_kerja'] =  $data_perubahan['tanggal_lahir_pencari_kerja'];
            $this->bidang_kolom['tempat_lahir_pencari_kerja'] =  $data_perubahan['tempat_lahir_pencari_kerja'];
            $this->bidang_kolom['kewarganegaraan_pencari_kerja'] =  $data_perubahan['kewarganegaraan_pencari_kerja'];
            $this->bidang_kolom['agama_pencari_kerja'] =  $data_perubahan['agama_pencari_kerja'];
            $this->bidang_kolom['pekerjaan_pencari_kerja'] =  $data_perubahan['pekerjaan_pencari_kerja'];
            $this->bidang_kolom['gol_darah_pencari_kerja'] =  $data_perubahan['gol_darah_pencari_kerja'];
            $this->bidang_kolom['nomor_ktp_pencari_kerja'] =  $data_perubahan['nomor_ktp_pencari_kerja'];
            $this->bidang_kolom['alamat_pencari_kerja'] = $data_perubahan['alamat_pencari_kerja'];
            $this->bidang_kolom['tujuan_membuat_surat_pencari_kerja'] = $data_perubahan['tujuan_membuat_surat_pencari_kerja'];
        }
        if (isset($template_dokumen['template_dokumen']) && $template_dokumen['template_dokumen'] != '') {
            foreach ($this->bidang_kolom as $key => $value) {
                $data = $value;

                if ($key == 'jenis_kelamin_pencari_kerja') {
                    if ($value == 1) {
                        $data = "LAKI-LAKI";
                    } else {
                        $data = "PEREMPUAN";
        ...
        $this->data['v']['page'] = 'surat_pencari_kerja_editor';
        $this->data['v']['script'] = 'surat_pencari_kerja_editor';
