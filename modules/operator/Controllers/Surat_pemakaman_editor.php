...
class Surat_pemakaman_editor extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        // foreach (nama_teks('surat_keterangan_pemakaman') as $key => $value) {
        //    $this->bidang_kolom[$value] = '(.....X.....)';
        // }

        //ambil kode kata untuk membuat option
        $this->list_kode_kata = ['jenis_pemakaman', 'jenis_kelamin', 'agama', 'jenis_pekerjaan', 'gol_darah'];
        ...
        if ($param != '' && $id_user) {
            $data_surat_pemakaman = $this->tbl_surat_pemakaman->where(['id_surat_pemakaman' => intval($param), 'status_surat_pemakaman' => 2])->first();
        ...
        if ($data_surat_pemakaman && $data_diri) {
            $data_perubahan = $data_surat_pemakaman;
            $this->data['data_surat']['id_surat'] = $data_surat_pemakaman['id_surat_pemakaman'];
        } else {
        ...
        $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_keterangan_pemakaman', 'default_template' => 1])->first();
        if ($template_dokumen) {
        } else {
            $template_dokumen = $this->tbl_template_dokumen->where(['jenis_template' => 'surat_keterangan_pemakaman'])->first();
        }

        if (isset($data_perubahan)) {
            $this->bidang_kolom['hari_wafat'] = ubah_tanggal($data_perubahan['hari_wafat'])['hari'];
            $this->bidang_kolom['tanggal_wafat_pemakaman'] = date('d-m-Y', strtotime($data_perubahan['tanggal_wafat_pemakaman']));
            $this->bidang_kolom['jam_wafat'] =  substr($data_perubahan['jam_wafat'], 0, -3);
            $this->bidang_kolom['tempat_wafat'] = $data_perubahan['tempat_wafat'];
            $this->bidang_kolom['penyebab_wafat'] = $data_perubahan['penyebab_wafat'];

            //Data pemakaman

            $this->bidang_kolom['nama_pemakaman'] = $data_perubahan['nama_pemakaman'];
            $this->bidang_kolom['jenis_kelamin_pemakaman'] = $data_perubahan['jenis_kelamin_pemakaman'];
            $this->bidang_kolom['umur_pemakaman'] =  $data_perubahan['umur_pemakaman'];
            $this->bidang_kolom['kewarganegaraan_pemakaman'] =  $data_perubahan['kewarganegaraan_pemakaman'];
            $this->bidang_kolom['agama_pemakaman'] =  $data_perubahan['agama_pemakaman'];
            $this->bidang_kolom['pekerjaan_pemakaman'] =  $data_perubahan['pekerjaan_pemakaman'];
            $this->bidang_kolom['alamat_pemakaman'] = $data_perubahan['alamat_pemakaman'];
            $this->bidang_kolom['tempat_dimakamkan'] = $data_perubahan['tempat_dimakamkan'];
        }
        if (isset($template_dokumen['template_dokumen']) && $template_dokumen['template_dokumen'] != '') {
            foreach ($this->bidang_kolom as $key => $value) {
                $data = $value;

                if ($key == 'jenis_kelamin_pemakaman') {
                    if ($value == 1) {
                        $data = "LAKI-LAKI";
                    } else {
                        $data = "PEREMPUAN";
        ...
        $this->data['v']['page'] = 'surat_pemakaman_editor';
        $this->data['v']['script'] = 'surat_pemakaman_editor';
