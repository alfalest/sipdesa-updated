...
class Surat_ket_tidak_mampu extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if (isset($this->get['cari']) && $this->get['cari'] != '') {
            ...
            $this->data['kk_proses'] = $this->tbl_surat_ket_tidak_mampu->where(['status_surat_ket_tidak_mampu' => $status_perubahan])
                ->like('nama_ket_tidak_mampu', $this->get['cari'])
                ->orderBy("update_time", "ASC")
                ->paginate($per_page, 'sk');
        } else {
            $this->data['kk_proses'] = $this->tbl_surat_ket_tidak_mampu->where(['status_surat_ket_tidak_mampu' => $status_perubahan])->orderBy("update_time", $urutan)->paginate($per_page, 'sk');
        }
