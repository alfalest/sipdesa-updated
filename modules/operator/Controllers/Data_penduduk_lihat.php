...
use DateTime;
class Data_penduduk_lihat extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        $this->data['data_penduduk'] = [];
        if($param != ''){
            $db_penduduk = $this->tbl_master_kk->where(['tbl_master_nik.id_master_nik' => intval($param)])->join('tbl_master_nik', 'tbl_master_kk.nkk = tbl_master_nik.nkk')->first();
            if($db_penduduk){
                $this->data['data_penduduk'] = $db_penduduk;
            } else {
                return redirect()->to( base_url().'/operator/data-penduduk');
                exit;
            }
        } else {
            return redirect()->to( base_url().'/operator/data-penduduk');
        ...
        if(count($this->data['data_penduduk']) != 0){
                $this->data['data_penduduk'] = $this->kode_kata($this->data['data_penduduk']);
                $this->data['data_penduduk'] = $this->negara($this->data['data_penduduk']);
                $this->data['data_penduduk'] = $this->wilayah($this->data['data_penduduk']);
        }

        if(isset($this->data['data_penduduk']['tanggal_lahir'])){
            $this->data['data_penduduk']['umur'] = $this->umur($this->data['data_penduduk']['tanggal_lahir']);
        }

        $this->data['v']['page'] = 'data_penduduk_lihat';
        $this->data['v']['script'] = 'data_penduduk_lihat';
