...
class Surat_catatan_kepolisian_editor_update extends BaseController
{
    public $post;
    ...
    public function index()
    {
        ...
        $set_data = [
            ...
            'jenis_surat' => 'surat_keterangan_catatan_kepolisian',
            'teks_isi_surat' => '',
            'nomor_surat' => '',
            'nama_penanda_tangan' => '',
        ...
        if (isset($this->post['id_surat']) && $this->post['id_surat'] != '') {
            $data_surat_catatan_kepolisian = $this->tbl_surat_catatan_kepolisian->where(['id_surat_catatan_kepolisian' => $this->post['id_surat'], 'status_surat_catatan_kepolisian' => 2])->first();
            if ($data_surat_catatan_kepolisian) {
                $id_surat_catatan_kepolisian =  $data_surat_catatan_kepolisian['id_surat_catatan_kepolisian'];
                $set_data['id_surat'] = $data_surat_catatan_kepolisian['id_surat_catatan_kepolisian'];
            } else {
                $valid = false;
        ...
        if ($id_surat_catatan_kepolisian != '' && $id_surat_catatan_kepolisian > 0) {
            if (isset($id_data_surat) && $id_data_surat != '' && $this->message_info['status']) {
                ...
                return redirect()->to(base_url() . '/operator/surat-catatan-kepolisian-editor/' . $id_surat_catatan_kepolisian);
            }
        } else {
            //return redirect()->to( base_url().'/operator/surat-catatan_kepolisian/diterima');
            return redirect()->to(base_url() . '/operator/surat-catatan-kepolisian/diterima');
        }
        exit;
    }
