...
class Surat_ket_janda_update extends BaseController
{
    ...
    public function index()
    {
        ...
        $set_tiket = [
            'nomor_tiket' => '',
            'jenis_tiket' => 'surat_keterangan_status_janda',
            'filename_surat' => 'surat-ket-janda-lihat',
            'id_surat' => '',
            'status_tiket' => 1,
            'id_user' => '',
        ...
        if (isset($this->post['id_surat_ket_janda']) && $this->post['id_surat_ket_janda'] != '') {
            $data_surat_ket_janda = $this->tbl_surat_ket_janda->where(['id_surat_ket_janda' => $this->post['id_surat_ket_janda'], 'status_surat_ket_janda' => 1])->first();
            if ($data_surat_ket_janda) {
                $id_surat_ket_janda = $this->post['id_surat_ket_janda'];
                $set_tiket['id_surat'] = $data_surat_ket_janda['id_surat_ket_janda'];
                $set_tiket['id_user'] = $data_surat_ket_janda['id_user'];
            } else {
                $valid = false;
                $this->message_info['text'] .= 'Surat ket_janda tidak ditemukan<br>';
            }
        }
        ...
        //validation
        if (isset($this->post['status_surat_ket_janda']) && in_array($this->post['status_surat_ket_janda'], [2, 3])) {
            if ($this->post['status_surat_ket_janda'] == 2) {
                if (isset($data_surat_ket_janda['id_user']) && $data_surat_ket_janda['id_user'] != 0) {
            ...
            $set_data['status_surat_ket_janda'] = $this->post['status_surat_ket_janda'];
        } else {
            $valid = false;
            $this->message_info['text'] .= 'Pilih salah satu jenis verifikasi : Terima atau Tolak<br>';
            $this->invalid_info['status_surat_ket_janda'] = 'Pilih salah satu jenis verifikasi : Terima atau Tolak';
        }
        ...
        if (isset($this->post['keterangan_operator']) &&  $this->post['keterangan_operator'] != '') {
            if (mb_strlen($this->post['keterangan_operator'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_ket_janda'] = 'Catatan melebihi 150 karakter';
            }
        }
        ...
        if (isset($this->post['keterangan_tiket']) && $this->post['keterangan_tiket'] !=  '') {
            if (mb_strlen($this->post['keterangan_tiket'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_ket_janda'] = 'Keterangan tiket melebihi 150 karakter';
            }
        }
        ...
        if ($valid) {
            if ($set_data['status_surat_ket_janda'] == 2) {
            ...
            if ($valid) {
                if ($this->tbl_surat_ket_janda->set($set_data)->where('id_surat_ket_janda', $id_surat_ket_janda)->update()) {
                    ...
                    $this->message_info['text'] .= 'Data surat ket_janda berhasil diubah<br>';
                    return redirect()->to(base_url() . '/operator/surat-ket-janda-edit/' . $id_surat_ket_janda);
                } else {
                    $this->message_info['text'] .= 'Data surat ket_janda gagal diverifikasi, periksa jika ada tiket atas surat ini<br>';
                }
            }
        }
        ...
        if ($id_surat_ket_janda != '' && $id_surat_ket_janda > 0) {
            if ($this->message_info['status']) {
                return redirect()->to(base_url() . '/operator/surat-ket-janda-lihat/' . $id_surat_ket_janda);
            } else {
                return redirect()->to(base_url() . '/operator/surat-ket-janda-edit/' . $id_surat_ket_janda);
            }
        } else {
