...
class Surat_kematian_lihat extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->list_kode_kata = [
            'tempat_dilahirkan', 'jenis_kematian', 'penolong_kematian', 'jenis_kelamin',
            'agama', 'jenis_pekerjaan', 'status_perkawinan', 'gol_darah',
            'tempat_dilahirkan', 'jenis_kematian', 'penolong_kematian'
        ];
        $this->arr_postfix = ['_anak' => 'Anak', '_ayah' => 'Ayah', '_ibu' => 'Ibu', '_kematian' => 'Kematian'];
    }
    ...
    public function index($param = '')
    {
        ...
        if ($param != '' && $id_user) {
            $data_surat_kematian = $this->tbl_surat_kematian->where(['id_surat_kematian' => intval($param)])->first();
        }

        if ($data_surat_kematian) {
            $this->data['data_surat_kematian'] = $data_surat_kematian;

            if (isset($this->data['data_surat_kematian']['id_user'])) {
                $this->data['data_user_pemohon'] = $this->tbl_user->where(['id_user' => $this->data['data_surat_kematian']['id_user']])->first();
                $this->data['data_diri_pemohon'] = $this->tbl_data_diri->where(['id_user' => $this->data['data_surat_kematian']['id_user']])->first();
        ...
        if (!isset($this->data['data_surat_kematian']['link_foto_kk']) || $this->data['data_surat_kematian']['link_foto_kk'] == '') {
            if (isset($data_surat_kematian['link_foto_kk'])) {
                $this->data['data_surat_kematian']['link_foto_kk'] = $data_surat_kematian['link_foto_kk'];
            }
        }

        //ambil data wilayah untuk membuat option
        if (isset($this->data['data_surat_kematian'])) {
            foreach ($this->arr_postfix as $key => $value) {
                $this->data['data_surat_kematian'] = $this->wilayah($this->data['data_surat_kematian'], $key);
                $this->data['data_surat_kematian'] = $this->kode_kata($this->data['data_surat_kematian'], $key);
                $this->data['data_surat_kematian'] = $this->negara($this->data['data_surat_kematian'], $key);
                $this->data['data_surat_kematian'] = $this->kode_kata($this->data['data_surat_kematian']);
            }
        }
        ...
        $this->data['v']['page'] = 'surat_kematian_lihat';
        $this->data['v']['script'] = 'surat_kematian_lihat';
