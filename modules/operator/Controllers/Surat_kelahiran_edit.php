...
class Surat_kelahiran_edit extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if($param != '' && $id_user){
            $data_surat_kelahiran = $this->tbl_surat_kelahiran->where(['id_surat_kelahiran' => intval($param), 'status_surat_kelahiran' => 1])->first();
        }
        ...
        if($this->session->getFlashdata('inp_val')){
            foreach ($this->session->getFlashdata('inp_val') as $key => $value) {
                if($key != 'id_surat_kelahiran'){
                    $this->data['data_surat_kelahiran'][$key] = $value;
                }
            }
        }
