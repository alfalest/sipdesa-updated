...
class Surat_kematian_update extends BaseController
{
    ...
    public function index()
    {
        ...
        $set_tiket = [
            'nomor_tiket' => '',
            'jenis_tiket' => 'surat_keterangan_kematian',
        ...
        if (isset($this->post['id_surat_kematian']) && $this->post['id_surat_kematian'] != '') {
            $data_surat_kematian = $this->tbl_surat_kematian->where(['id_surat_kematian' => $this->post['id_surat_kematian'], 'status_surat_kematian' => 1])->first();
            if ($data_surat_kematian) {
                $id_surat_kematian = $this->post['id_surat_kematian'];
                $set_tiket['id_surat'] = $data_surat_kematian['id_surat_kematian'];
                $set_tiket['id_user'] = $data_surat_kematian['id_user'];
            } else {
                $valid = false;
        ...
        //validation
        if (isset($this->post['status_surat_kematian']) && in_array($this->post['status_surat_kematian'], [2, 3])) {
            if ($this->post['status_surat_kematian'] == 2) {
                if (isset($data_surat_kematian['id_user']) && $data_surat_kematian['id_user'] != 0) {
            ...
            $set_data['status_surat_kematian'] = $this->post['status_surat_kematian'];
        } else {
            $valid = false;
            $this->message_info['text'] .= 'Pilih salah satu jenis verifikasi : Terima atau Tolak<br>';
            $this->invalid_info['status_surat_kematian'] = 'Pilih salah satu jenis verifikasi : Terima atau Tolak';
        }
        ...
        if (isset($this->post['keterangan_operator']) &&  $this->post['keterangan_operator'] != '') {
            if (mb_strlen($this->post['keterangan_operator'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_kematian'] = 'Catatan melebihi 150 karakter';
            }
        }
        ...
        if (isset($this->post['keterangan_tiket']) && $this->post['keterangan_tiket'] !=  '') {
            if (mb_strlen($this->post['keterangan_tiket'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_kematian'] = 'Keterangan tiket melebihi 150 karakter';
            }
        }
        ...
        if ($valid) {
            if ($set_data['status_surat_kematian'] == 2) {
            ...
            if ($valid) {
                if ($this->tbl_surat_kematian->set($set_data)->where('id_surat_kematian', $id_surat_kematian)->update()) {
                    ...
                    return redirect()->to(base_url() . '/operator/surat-kematian-edit/' . $id_surat_kematian);
                } else {
                    $this->message_info['text'] .= 'Data surat kematian gagal diverifikasi, periksa jika ada tiket atas surat ini<br>';
                }
            }
        }
        ...
        if ($id_surat_kematian != '' && $id_surat_kematian > 0) {
            if ($this->message_info['status']) {
                return redirect()->to(base_url() . '/operator/surat-kematian-lihat/' . $id_surat_kematian);
            } else {
                return redirect()->to(base_url() . '/operator/surat-kematian-edit/' . $id_surat_kematian);
            }
        } else {
