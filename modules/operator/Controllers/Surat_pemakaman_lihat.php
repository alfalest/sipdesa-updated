...
class Surat_pemakaman_lihat extends BaseController
{
    ...
    public function __construct()
    {
        ...
        $this->arr_postfix = ['_anak' => 'Anak', '_ayah' => 'Ayah', '_ibu' => 'Ibu', '_pemakaman' => 'Pemakaman'];
    }
    ...
    public function index($param = '')
    {
        ...
        if ($param != '' && $id_user) {
            $data_surat_pemakaman = $this->tbl_surat_pemakaman->where(['id_surat_pemakaman' => intval($param)])->first();
        }

        if ($data_surat_pemakaman) {
            $this->data['data_surat_pemakaman'] = $data_surat_pemakaman;

            if (isset($this->data['data_surat_pemakaman']['id_user'])) {
                $this->data['data_user_pemohon'] = $this->tbl_user->where(['id_user' => $this->data['data_surat_pemakaman']['id_user']])->first();
                $this->data['data_diri_pemohon'] = $this->tbl_data_diri->where(['id_user' => $this->data['data_surat_pemakaman']['id_user']])->first();
        ...
        if (!isset($this->data['data_surat_pemakaman']['link_foto_kk']) || $this->data['data_surat_pemakaman']['link_foto_kk'] == '') {
            if (isset($data_surat_pemakaman['link_foto_kk'])) {
                $this->data['data_surat_pemakaman']['link_foto_kk'] = $data_surat_pemakaman['link_foto_kk'];
            }
        }

        //ambil data wilayah untuk membuat option
        if (isset($this->data['data_surat_pemakaman'])) {
            foreach ($this->arr_postfix as $key => $value) {
                $this->data['data_surat_pemakaman'] = $this->wilayah($this->data['data_surat_pemakaman'], $key);
                $this->data['data_surat_pemakaman'] = $this->kode_kata($this->data['data_surat_pemakaman'], $key);
                $this->data['data_surat_pemakaman'] = $this->negara($this->data['data_surat_pemakaman'], $key);
                $this->data['data_surat_pemakaman'] = $this->kode_kata($this->data['data_surat_pemakaman']);
            }
        }
        ...
        $this->data['v']['page'] = 'surat_pemakaman_lihat';
        $this->data['v']['script'] = 'surat_pemakaman_lihat';
