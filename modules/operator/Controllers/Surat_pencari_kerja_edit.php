...
class Surat_pencari_kerja_edit extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        $this->list_kode_kata = ['tempat_dilahirkan', 'jenis_pencari_kerja', 'penolong_pencari_kerja', 'jenis_kelamin', 'agama', 'jenis_pekerjaan', 'status_perkawinan', 'gol_darah'];
        ...
        if ($param != '' && $id_user) {
            $data_surat_pencari_kerja = $this->tbl_surat_pencari_kerja->where(['id_surat_pencari_kerja' => intval($param), 'status_surat_pencari_kerja' => 1])->first();
        }
        ...
        if ($this->session->getFlashdata('inp_val')) {
            foreach ($this->session->getFlashdata('inp_val') as $key => $value) {
                if ($key != 'id_surat_pencari_kerja') {
                    $this->data['data_surat_pencari_kerja'][$key] = $value;
                }
            }
        }
        ...
        if (isset($this->data['data_surat_pencari_kerja'])) {
            $this->data['data_surat_pencari_kerja'] = $this->arr_wilayah($this->data['data_surat_pencari_kerja'], '_anak');
            $this->data['data_surat_pencari_kerja'] = $this->arr_wilayah($this->data['data_surat_pencari_kerja'], '_ayah');
            $this->data['data_surat_pencari_kerja'] = $this->arr_wilayah($this->data['data_surat_pencari_kerja'], '_ibu');
        }
