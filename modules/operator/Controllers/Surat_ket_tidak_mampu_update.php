...
class Surat_ket_tidak_mampu_update extends BaseController
{
    ...
    public function index()
    {
        ...
        $set_tiket = [
            'nomor_tiket' => '',
            'jenis_tiket' => 'surat_keterangan_ket_tidak_mampu',
            'filename_surat' => 'surat-ket-tidak-mampu-lihat',
            'id_surat' => '',
            'status_tiket' => 1,
            'id_user' => '',
        ...
        if (isset($this->post['id_surat_ket_tidak_mampu']) && $this->post['id_surat_ket_tidak_mampu'] != '') {
            $data_surat_ket_tidak_mampu = $this->tbl_surat_ket_tidak_mampu->where(['id_surat_ket_tidak_mampu' => $this->post['id_surat_ket_tidak_mampu'], 'status_surat_ket_tidak_mampu' => 1])->first();
            if ($data_surat_ket_tidak_mampu) {
                $id_surat_ket_tidak_mampu = $this->post['id_surat_ket_tidak_mampu'];
                $set_tiket['id_surat'] = $data_surat_ket_tidak_mampu['id_surat_ket_tidak_mampu'];
                $set_tiket['id_user'] = $data_surat_ket_tidak_mampu['id_user'];
            } else {
                $valid = false;
                $this->message_info['text'] .= 'Surat ket_tidak_mampu tidak ditemukan<br>';
            }
        }
        ...
        //validation
        if (isset($this->post['status_surat_ket_tidak_mampu']) && in_array($this->post['status_surat_ket_tidak_mampu'], [2, 3])) {
            if ($this->post['status_surat_ket_tidak_mampu'] == 2) {
                if (isset($data_surat_ket_tidak_mampu['id_user']) && $data_surat_ket_tidak_mampu['id_user'] != 0) {
            ...
            $set_data['status_surat_ket_tidak_mampu'] = $this->post['status_surat_ket_tidak_mampu'];
        } else {
            $valid = false;
            $this->message_info['text'] .= 'Pilih salah satu jenis verifikasi : Terima atau Tolak<br>';
            $this->invalid_info['status_surat_ket_tidak_mampu'] = 'Pilih salah satu jenis verifikasi : Terima atau Tolak';
        }
        ...
        if (isset($this->post['keterangan_operator']) &&  $this->post['keterangan_operator'] != '') {
            if (mb_strlen($this->post['keterangan_operator'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_ket_tidak_mampu'] = 'Catatan melebihi 150 karakter';
            }
        }
        ...
        if (isset($this->post['keterangan_tiket']) && $this->post['keterangan_tiket'] !=  '') {
            if (mb_strlen($this->post['keterangan_tiket'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_ket_tidak_mampu'] = 'Keterangan tiket melebihi 150 karakter';
            }
        }
        ...
        if ($valid) {
            if ($set_data['status_surat_ket_tidak_mampu'] == 2) {
            ...
            if ($valid) {
                if ($this->tbl_surat_ket_tidak_mampu->set($set_data)->where('id_surat_ket_tidak_mampu', $id_surat_ket_tidak_mampu)->update()) {
                    ...
                    $this->message_info['text'] .= 'Data surat ket_tidak_mampu berhasil diubah<br>';
                    return redirect()->to(base_url() . '/operator/surat-ket-tidak-mampu-edit/' . $id_surat_ket_tidak_mampu);
                } else {
                    $this->message_info['text'] .= 'Data surat ket_tidak_mampu gagal diverifikasi, periksa jika ada tiket atas surat ini<br>';
                }
            }
        }
        ...
        if ($id_surat_ket_tidak_mampu != '' && $id_surat_ket_tidak_mampu > 0) {
            if ($this->message_info['status']) {
                return redirect()->to(base_url() . '/operator/surat-ket-tidak-mampu-lihat/' . $id_surat_ket_tidak_mampu);
            } else {
                return redirect()->to(base_url() . '/operator/surat-ket-tidak-mampu-edit/' . $id_surat_ket_tidak_mampu);
            }
        } else {
