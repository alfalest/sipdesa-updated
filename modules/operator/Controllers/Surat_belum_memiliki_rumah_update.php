...
class Surat_belum_memiliki_rumah_update extends BaseController
{
    ...
    public function index()
    {
        ...
        $set_tiket = [
            'nomor_tiket' => '',
            'jenis_tiket' => 'surat_keterangan_belum_memiliki_rumah',
            'filename_surat' => 'surat-belum-memiliki-rumah-lihat',
            'id_surat' => '',
            'status_tiket' => 1,
            'id_user' => '',
        ...
        if (isset($this->post['id_surat_belum_memiliki_rumah']) && $this->post['id_surat_belum_memiliki_rumah'] != '') {
            $data_surat_belum_memiliki_rumah = $this->tbl_surat_belum_memiliki_rumah->where(['id_surat_belum_memiliki_rumah' => $this->post['id_surat_belum_memiliki_rumah'], 'status_surat_belum_memiliki_rumah' => 1])->first();
            if ($data_surat_belum_memiliki_rumah) {
                $id_surat_belum_memiliki_rumah = $this->post['id_surat_belum_memiliki_rumah'];
                $set_tiket['id_surat'] = $data_surat_belum_memiliki_rumah['id_surat_belum_memiliki_rumah'];
                $set_tiket['id_user'] = $data_surat_belum_memiliki_rumah['id_user'];
            } else {
                $valid = false;
                $this->message_info['text'] .= 'Surat belum_memiliki_rumah tidak ditemukan<br>';
            }
        }
        ...
        //validation
        if (isset($this->post['status_surat_belum_memiliki_rumah']) && in_array($this->post['status_surat_belum_memiliki_rumah'], [2, 3])) {
            if ($this->post['status_surat_belum_memiliki_rumah'] == 2) {
                if (isset($data_surat_belum_memiliki_rumah['id_user']) && $data_surat_belum_memiliki_rumah['id_user'] != 0) {
            ...
            $set_data['status_surat_belum_memiliki_rumah'] = $this->post['status_surat_belum_memiliki_rumah'];
        } else {
            $valid = false;
            $this->message_info['text'] .= 'Pilih salah satu jenis verifikasi : Terima atau Tolak<br>';
            $this->invalid_info['status_surat_belum_memiliki_rumah'] = 'Pilih salah satu jenis verifikasi : Terima atau Tolak';
        }
        ...
        if (isset($this->post['keterangan_operator']) &&  $this->post['keterangan_operator'] != '') {
            if (mb_strlen($this->post['keterangan_operator'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_belum_memiliki_rumah'] = 'Catatan melebihi 150 karakter';
            }
        }
        ...
        if (isset($this->post['keterangan_tiket']) && $this->post['keterangan_tiket'] !=  '') {
            if (mb_strlen($this->post['keterangan_tiket'], "UTF-8") <= 150) {
                ...
                $this->invalid_info['status_surat_belum_memiliki_rumah'] = 'Keterangan tiket melebihi 150 karakter';
            }
        }
        ...
        if ($valid) {
            if ($set_data['status_surat_belum_memiliki_rumah'] == 2) {
            ...
            if ($valid) {
                if ($this->tbl_surat_belum_memiliki_rumah->set($set_data)->where('id_surat_belum_memiliki_rumah', $id_surat_belum_memiliki_rumah)->update()) {
                    ...
                    $this->message_info['text'] .= 'Data surat belum_memiliki_rumah berhasil diubah<br>';
                    return redirect()->to(base_url() . '/operator/surat-belum-memiliki-rumah-edit/' . $id_surat_belum_memiliki_rumah);
                } else {
                    $this->message_info['text'] .= 'Data surat belum_memiliki_rumah gagal diverifikasi, periksa jika ada tiket atas surat ini<br>';
                }
            }
        }
        ...
        if ($id_surat_belum_memiliki_rumah != '' && $id_surat_belum_memiliki_rumah > 0) {
            if ($this->message_info['status']) {
                return redirect()->to(base_url() . '/operator/surat-belum-memiliki-rumah-lihat/' . $id_surat_belum_memiliki_rumah);
            } else {
                return redirect()->to(base_url() . '/operator/surat-belum-memiliki-rumah-edit/' . $id_surat_belum_memiliki_rumah);
            }
        } else {
