...
class Surat_kematian extends BaseController
{
    ...
    public function index($param = '')
    {
        ...
        if (isset($this->get['cari']) && $this->get['cari'] != '') {
            ...
            $this->data['kk_proses'] = $this->tbl_surat_kematian->where(['status_surat_kematian' => $status_perubahan])
                ->like('nama_kematian', $this->get['cari'])
                ->orderBy("update_time", "ASC")
                ->paginate($per_page, 'sk');
        } else {
            $this->data['kk_proses'] = $this->tbl_surat_kematian->where(['status_surat_kematian' => $status_perubahan])->orderBy("update_time", $urutan)->paginate($per_page, 'sk');
        }
