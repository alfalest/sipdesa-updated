...
class Surat_kelahiran_update extends BaseController
{
    ...
    public function index()
    {
        ...
        $set_tiket = [
            'nomor_tiket' => '',
            'jenis_tiket' => 'surat_keterangan_kelahiran',
            'filename_surat' => 'surat-kelahiran-lihat',
            'id_surat' => '',
            'status_tiket' => 1,
            'id_user' => '',
        ...
        if(isset($this->post['id_surat_kelahiran']) && $this->post['id_surat_kelahiran'] != '' ){
            $data_surat_kelahiran = $this->tbl_surat_kelahiran->where(['id_surat_kelahiran' => $this->post['id_surat_kelahiran'] , 'status_surat_kelahiran' => 1])->first();
            if($data_surat_kelahiran){
                $id_surat_kelahiran = $this->post['id_surat_kelahiran'];
                $set_tiket['id_surat'] = $data_surat_kelahiran['id_surat_kelahiran'];
                $set_tiket['id_user'] = $data_surat_kelahiran['id_user'];
            } else {
                $valid = false;
                $this->message_info['text'] .= 'Surat kelahiran tidak ditemukan<br>';
            }
        }
        ...
        //validation
        if(isset($this->post['status_surat_kelahiran']) && in_array($this->post['status_surat_kelahiran'], [2,3]) ){
            if($this->post['status_surat_kelahiran'] == 2){
                if(isset($data_surat_kelahiran['id_user']) && $data_surat_kelahiran['id_user'] != 0){
            ...
            $set_data['status_surat_kelahiran'] = $this->post['status_surat_kelahiran'];
        } else {
            $valid = false;
            $this->message_info['text'] .= 'Pilih salah satu jenis verifikasi : Terima atau Tolak<br>';
            $this->invalid_info['status_surat_kelahiran'] = 'Pilih salah satu jenis verifikasi : Terima atau Tolak';
        }
        ...
        if(isset($this->post['keterangan_operator']) &&  $this->post['keterangan_operator'] != '' ){
            if(mb_strlen($this->post['keterangan_operator'] , "UTF-8") <= 150 ){
                ...
                $this->invalid_info['status_surat_kelahiran'] = 'Catatan melebihi 150 karakter';
            }
        }
        ...
        if(isset($this->post['keterangan_tiket']) && $this->post['keterangan_tiket'] !=  ''  ){
            if( mb_strlen($this->post['keterangan_tiket'] , "UTF-8") <= 150 ){
                ...
                $this->invalid_info['status_surat_kelahiran'] = 'Keterangan tiket melebihi 150 karakter';
            }
        }
...
$arr_post_photo = [ 'foto_ktp_ayah', 'foto_ktp_ibu', 'foto_kartu_keluarga', 'foto_buku_nikah', 'foto_akta_perkawinan' ];
                ...
                if($valid){
                    if($set_data['status_surat_kelahiran'] == 2){
                    ...
                    if($valid){
                        if($this->tbl_surat_kelahiran->set($set_data)->where('id_surat_kelahiran', $id_surat_kelahiran)->update()){
                            ...
                            return redirect()->to( base_url().'/operator/surat-kelahiran-edit/'.$id_surat_kelahiran);
                        } else {
                            $this->message_info['text'] .= 'Data surat kelahiran gagal diverifikasi, periksa jika ada tiket atas surat ini<br>';
                        }
                    }
                }
        ...
        if($id_surat_kelahiran != '' && $id_surat_kelahiran > 0){
            if($this->message_info['status']){
                return redirect()->to( base_url().'/operator/surat-kelahiran-lihat/'.$id_surat_kelahiran);
            } else {
                return redirect()->to( base_url().'/operator/surat-kelahiran-edit/'.$id_surat_kelahiran);
            }
        } else {
