          ...
          <div class="card-body px-1 px-sm-2 px-md-3 px-lg-4">
            <div class="row">
              <div class="col-12 d-flex justify-content-between">
                <p class="font-weight-bold text-lg m-0">Surat Pengantar Pencari Kerja</p>
                <div>
            ...
            <div class="row">
              <div class="col-12">
                ...
                <?php if (isset($inp_val['cari']) && $inp_val['cari'] != '') : ?>
                  <div class="mb-3">
                    Pencarian: <a href="<?= base_url() ?>/operator/surat-pencari-kerja" class="text-danger"><?= $inp_val['cari'] ?> <i class="fas fa-times"></i></a>
                  </div>
                <?php endif; ?>
              </div>
            </div>
            ...
            <div class="row">
              <div class="col-12">
                <div class="d-flex justify-content-between">
                  <a class="border shadow-sm btn btn-block btn-light my-0 mx-1 round-1 text-primary" href="<?= base_url() ?>/operator/surat-pencari-kerja/diproses">
                    Diproses <i class="fas fa-angle-right"></i>
                  </a>
                  <a class="border shadow-sm btn btn-block btn-light  my-0 mx-1 round-1 text-primary" href="<?= base_url() ?>/operator/surat-pencari-kerja/diterima">
                    Diterima <i class="fas fa-angle-right"></i>
                  </a>
                  <a class="border shadow-sm btn btn-block btn-light  my-0 mx-1 round-1 text-primary" href="<?= base_url() ?>/operator/surat-pencari-kerja/ditolak">
                    ...
                    <div class="border shadow-sm round-1 d-flex flex-column mb-3 p-2">
                      <div class="d-flex justify-content-between border-bottom pb-2">
                        <div class="d-flex flex-row">
                          <div class="mr-3">
                            <?= $status_surat_pencari_kerja[$value['status_surat_pencari_kerja']] ?>
                          </div>
                          <div class="text-sm text-muted"><?= date('H:i:s , d-m-Y', strtotime($value['update_time'])) ?></div>
                        </div>
                        <div class="">
                          <?php if ($value['status_surat_pencari_kerja'] == 1) : ?>
                            <a href="<?= base_url() ?>/operator/surat-pencari-kerja-lihat/<?= $value['id_surat_pencari_kerja'] ?>" class="px-3 py-0 btn btn-sm btn-success round-1 text-sm"><i class="fas fa-eye mr-2"></i> Lihat</a>

                          <?php elseif ($value['status_surat_pencari_kerja'] == 2) : ?>
                            <a href="<?= base_url() ?>/operator/surat-pencari-kerja-editor/<?= $value['id_surat_pencari_kerja'] ?>" class="px-3 py-0 btn btn-sm bg-indigo round-1 text-sm"><i class="fas fa-print mr-2"></i> Redaksi Editor</a>
                          <?php endif; ?>
                        </div>
                      </div>
                      <a href="<?= base_url() ?>/operator/surat-pencari-kerja-lihat/<?= $value['id_surat_pencari_kerja'] ?>">
                        <div class="row text-dark">
                          <div class="col-6 col-lg-3 mb-1">
                            <span class="text-muted font-weight-bold">Nama Pelapor</span><br>
                            <?= $value['nama_pencari_kerja'] ?>
                          </div>
                          <div class="col-6 col-lg-3 mb-1">
                            <span class="text-muted font-weight-bold">Tempat/Tanggal Lahir</span><br>
                            <?= $value['tempat_lahir_pencari_kerja'] ?>/<?= $value['tanggal_lahir_pencari_kerja'] ?>
                          </div>
                          <div class="col-6 col-lg-3 mb-1">
                            <span class="text-muted font-weight-bold">Alamat Pelapor</span><br>
                            <?= $value['alamat_pencari_kerja'] ?>
                          </div>
                        </div>
                      </a>
                    </div>
