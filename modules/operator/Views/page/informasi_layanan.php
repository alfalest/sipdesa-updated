            ...
            <div class="card round-1">
              <div class="card-header pr-3">
                <div class="d-flex justify-content-between">
                  <div>
                  <p class="font-weight-bold text-lg m-0"><?= (isset($title_page)) ? $title_page : 'Informasi Layanan' ?></p>
                  </div>
                  <div>
                  <?php if(isset($title_page)) : ?>
                    <a href="<?=base_url()?>/operator/informasi-layanan" class="btn btn-primary btn-sm rounded-pill"><i class="fas fa-times"></i></a>
                  <?php else : ?>
...
<?php if(isset($title_page)) : ?>
  <div class="card-body px-1 px-sm-2 px-md-3 px-lg-4">
            <div class="row">
              <div class="col-12">
                ...
                <?php if(isset($inp_val['cari']) && $inp_val['cari'] != '') : ?>
                  <a href="<?= base_url() ?>/operator/<?= (isset($link_back)) ? $link_back : '' ?>" class="text-danger"><?= $inp_val['cari'] ?> <i class="fas fa-times"></i></a>
                <?php endif; ?>
                </div>
              </div>
            </div>
            ...
            <div class="col-12">
            <div class="py-1 px-1">
            <a href="<?=base_url()?>/operator/<?= (isset($link_edit)) ? $link_edit : '' ?>" class="btn btn-primary btn-sm rounded-pill"><i class="mr-2 fas fa-plus"></i> Informasi Baru</a>

            </div>
            </div>
            </div>
            <br>
            <?php if( isset($informasi_layanan) && count($informasi_layanan) ) :?>
            <div class="row">
              <div id="kartu_keluarga_list" class="col-12">
                <?php foreach($informasi_layanan as $value) : ?>
                <div class="border shadow-sm round-1 d-flex flex-column mb-3 p-2">
                  <div class="d-flex justify-content-between border-bottom pb-2">
                    <div class="d-flex flex-row">
                      ...
                      <?php if($value['default_informasi']) : ?>
                    ...
                    <div class="">
                      <a href="<?= base_url() ?>/operator/<?= $link_edit ?>/<?=$value['id_informasi_layanan']?>" class="px-3 py-0 btn btn-sm btn-success round-1 text-sm"><i class="fas fa-edit mr-2"></i> Edit</a>
                    
                    </div>
                  </div>
                  <a href="<?=base_url()?>/operator/<?= $link_lihat ?>/<?=$value['id_informasi_layanan']?>">
                    <div class="row text-dark">
                      <div class="col-12 col-lg-6">
                        <div class="row h-100">
                          <div class="col-lg-12 col-6">
                            <span class="text-muted">Nama Informasi</span>
                          </div>
                          <div class="col-lg-12 col-6">
                            <span class="text-muted font-weight-bold"><?=$value['nama_informasi']?></span>
                          </div>
                        </div>
                      </div>
            ...
            <div class="row">
              <div class="col-12">
                <div class="d-flex justify-content-center"><?= (isset($pager) ) ? $pager->links('informasi_layanan', 'sipdesa_pager') : "" ?></div>
              </div>
            </div>
            <?php else : ?>
            <div class="row">
...
<?php else : ?>
          ...
          <a class="btn btn-block my-0 mx-1" href="<?= base_url() ?>/operator/informasi-layanan/<?= str_replace('_', '-', $key) ?>">
            <div class="d-flex justify-content-between">
              ...
              <?php if(isset($badge_notif['informasi_layanan']) && $badge_notif['informasi_layanan'] != 0) : ?>
                  <span class="badge badge-danger mr-3"><?= $badge_notif['informasi_layanan'] ?></span>
                <?php endif; ?>
